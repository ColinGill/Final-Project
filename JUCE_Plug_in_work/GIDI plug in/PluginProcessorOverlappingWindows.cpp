...
template <typename FloatType>
void JuceDemoPluginAudioProcessor::processBuffer (AudioBuffer<FloatType>& buffer, MidiBuffer& midi)
{

    const int numSamples = buffer.getNumSamples() , olf =2;
	float fund = 0, power = 0, maxValue =0;
	static int j= 0;
	int delayPos = 0, velRange = 50;
	auto sum = 0;
	...
	for (int channel = 0; channel < getTotalNumOutputChannels(); ++channel)
	{
		...
		//overlap 512 chunks in 1024 size buffer		
		...
		/*
		for (int fillHalfBuffer = 0; fillHalfBuffer < numSamples*olf; fillHalfBuffer++)
		{
			bufferSize_2[fillHalfBuffer] = channelData[fillHalfBuffer];
		}
		*/
	
			for (int samples = numSamples; samples < numSamples*olf; samples++)
			{
				power = power + abs(bufferSize_2[samples]);
			...
			if (power <= 10)
			{
				//updateGui(String(numSamples));
			}
		
			//updateGui(String(bufferSize_2[1023]) + ".." + String(bufferSize_2[0]));
			
			else
			{
				for (int n = 0; n < numSamples*olf; n = n + dsf)
				{

					for (int i = n; i < numSamples*olf; i = i + dsf)
					{
						sum = sum + (bufferSize_2[i] * bufferSize_2[i - n]);
					}
				...
				if (noteFound == 0)
				{
					//updateGui("nosn" + String(numSamples));
					currentNote = lastNoteValue;
				}
				else
				{
					//updateGui("nos:" + String(numSamples));
				}
			}
			
	}

	auto noteDuration = numSamples;
	MidiMessage msg;
	...
	midi.clear();
		...
		{
			velocity = (uint8)((maxValue * 127) * velRange);
			...
			//updateGui("midi velocity: " + String(velocity));
			midi.addEvent(MidiMessage::noteOff(1, lastNoteValue), 0);
			midi.addEvent(MidiMessage::noteOn(1, currentNote, (uint8)127), 0);
